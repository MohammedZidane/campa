...
import numpy as np
from skimage.transform import resize  # You might need to install scikit-image if not already available
...
class MPPData:
    ...
    def _from_data_dir(
        ...
        **kwargs: Any,
        ...
        base_dir = '/home/mohammed/campamodified/notebooks/example_data'
        print('data_dir:', data_dir)
        print('base_dir:', base_dir)
        metadata = pd.read_csv('/home/mohammed/campamodified/notebooks/example_data/2D_TNBC_unperturbed/train/metadata.csv', index_col=0).reset_index(drop=True)
        print('!!!!!!!!!!the line above me is instead the line below me!!!!!!!!!!!!!!!')
    ...
    def center_mpp(self) -> np.ndarray:
        ...
        """
        print('self.mpp.shape inside:', self.mpp.shape)
        print('self.mpp:', self.mpp)
        # print('self.mpp[1] inside:', self.mpp[1])
        c = int(self.mpp.shape[1] // 2)
        print('self.mpp[:, c, c, :]:', self.mpp[:, c, c, :])
        print('c:', c)
    ...
    def subset(
        ...
        **kwargs: Any,
        ...
        print('len(selected_obj_ids)0000000000000:', len(selected_obj_ids))
        print('selected_obj_ids00000000000:', selected_obj_ids)
        ...
        if obj_ids is not None:
            ...
            print('len(selected_obj_ids)!!!!!!!!!!!!!:', len(selected_obj_ids))
        ...
        for key, value in kwargs.items():
            ...
            print('!!!!commented out the the .loc[selected_obj_ids] above me to remove reptition of indices of obj_ids and avoid the error!!!!')
            print('self.data_config.OBJ_ID:', self.data_config.OBJ_ID)
            print('self.metadata.set_index(self.data_config.OBJ_ID):', self.metadata.set_index(self.data_config.OBJ_ID))
            print('self.metadata.set_index(self.data_config.OBJ_ID).loc[selected_obj_ids]:', self.metadata.set_index(self.data_config.OBJ_ID).loc[selected_obj_ids])
            assert key in cur_metadata.columns, f"provided column {key} was not found in the metadata table!"
            if value == "NO_NAN":
                print('cur_metadata:', cur_metadata)
                print('key:', key)
                print('cur_metadata[key]:', cur_metadata[key])
            ...
            print('mask:', mask)
            print('selected_obj_ids:', selected_obj_ids)
            selected_obj_ids = selected_obj_ids[mask]
            print('len(selected_obj_ids)0000000000000:', len(selected_obj_ids))
        # select ids based on nona conditions
        if nona_condition:  # TODO test this!
            if self.conditions is not None:
                mask = ~np.isnan(self.conditions).any(axis=0)
                print('changed the line above me from axis=1 to axis=0')
                print('!!!!commented out the the .loc[selected_obj_ids] below me to remove reptition of indices of obj_ids and avoid the error!!!!')           
                ...
                print('len(selected_obj_ids)1111111111111:', len(selected_obj_ids))
        ...
        if num is not None:
            ...
            print('len(selected_obj_ids)222222222222:', len(selected_obj_ids))
        # create mpp mask for applying subset
        obj_mask = self.metadata[self.data_config.OBJ_ID] #.isin(selected_obj_ids)
        print('!!!!!!!!!!commented out .isin(selected_obj_ids) above me!!!!!!!')
        mpp_mask = self._get_per_mpp_value(obj_mask)
        print('111111111111111111111')
        print('len(obj_mask000000000000):', len(obj_mask))
        print('mpp_mask000000000000:', mpp_mask)
        print('len(mpp_mask):', len(mpp_mask))
        # apply mask
    ...
    def subsample(
        ...
        if (frac is not None) or (num is not None):
            ...
            print('mpp_data in subsamle:', mpp_data)
    ...
    def add_neighborhood(self, size: int = 3, copy: bool = False, padding: bool = True) -> "MPPData":
        """
        ...
        mpp = self._get_neighborhood(self.obj_ids, self.x, self.y, size=size, padding=padding)
        print('self.center_mpp:', self.center_mpp)
        print('size // 2:', size // 2)
        print('mpp[:, size // 2, size // 2, :]:', mpp[:, size // 2, size // 2, :])
    ...
    def apply_mask(self, mask: np.ndarray, copy: bool = False) -> "MPPData":
        ...
        """
        data = {}
        print('self._data.keys():', self._data.keys())
        print('type(self._data[obj_ids]):', type(self._data['obj_ids']))
        print('self._data[obj_ids]:', self._data['obj_ids'])
    ...
    def _subtract_background(self, background_value: Union[float, List[float], str]) -> None:
        ...
        if isinstance(background_value, float):
            ...
            channels_metadata = pd.read_csv('/home/mohammed/campa/notebooks/example_data/raw/channels_metadata.csv')
            print('!!!!!!!commented out the line below me and hard coded the path above me!!!!!!!!!!!!')
            print('!!!!!!!commented out the lines of the channels_metadata file and adding the backgroudn info as I do not have htis data from the TNBC Parson lab!!!!!!')
    ...
    def _get_per_mpp_value(
        ...
        if isinstance(per_obj_value, dict):
            ...
            print('per_obj_df:', per_obj_df)
            print('per_obj_value in else:', per_obj_value)
        print('self.obj_ids:', self.obj_ids)
        ...
        print('type(per_mpp_value):', type(per_mpp_value))
        print('per_mpp_value:', per_mpp_value)
    ...
    def _get_neighborhood(
        ...
        if padding == True:
            ...
            print('data00000000:', data)
            print('np.unique(obj_ids):', np.unique(obj_ids))
            for obj_id in np.unique(obj_ids):
                mask = obj_ids == obj_id
                img, (xoff, yoff) = self.get_object_img(obj_id, data="mpp", pad=size // 2)
                print('xoff:', xoff)
                print('yoff:', yoff)
                print('img in the making:', img)
                vals = []
            ...
            for obj_id in np.unique(obj_ids):
                ...
                for x, y in zip(xs[mask], ys[mask]):
                    ...
                    vals.append(pad_border(img[idx], mode=border_mode))
                data[mask] = np.array(vals)
                print('data222222222222222222:', data)

                print('self.center_mpp222222222222222222222:', self.center_mpp)
            return data
    ...
    def _get_img_from_data(
        ...
        if img_size is not None:
            ...
            img = bbox.crop(img)
            print('img in _get_img_from_data in if:', img)
            print('img.shape in _get_img_from_data in if:', img.shape)
            return img  
        else:
            # offset info is only relevant when not cropping img to shape
            print('img in _get_img_from_data in else:', img)
            print('img.shape in _get_img_from_data in else:', img.shape)
            print('paaaaaaaaaaaaaaaaaaaaadddddddddddddddddddddddd:', pad)
            return img, (int(x.min() - pad), int(y.min() - pad))

        
    # def _get_img_from_data(
    #     x: np.ndarray, y: np.ndarray, data: np.ndarray, img_size: Optional[Tuple[int, int]] = None, pad: int = 0
    # ) -> Union[np.ndarray, Tuple[np.ndarray, Tuple[int, int]]]:
    #     print('finnnnnnalaallalaa')
    #     # Create image from x and y coordinates and fill with data
    #     # Your existing logic to form the initial image goes here
    
    #     # Example of creating a blank image and placing data points
    #     if img_size is None:
    #         # Determine the size based on x, y, and pad
    #         height = int(max(y) - min(y) + 1 + 2 * pad)
    #         width = int(max(x) - min(x) + 1 + 2 * pad)
    #         img = np.zeros((height, width, data.shape[1]))  # Assuming data has shape (n_coords, n_channels)
    #     else:
    #         # Use provided img_size
    #         height, width = img_size
    #         img = np.zeros((height, width, data.shape[1]))
    
    #     # Logic to populate img with data based on x, y, and data goes here
    #     # This is a crucial step and depends on how you intend to map data points to pixel values
    
    #     # If img_size was explicitly provided and does not match the calculated size,
    #     # Resize the image. Note: This might distort the image or alter pixel values.
    #     if img_size is not None and (img.shape[0] != height or img.shape[1] != width):
    #         img = resize(img, (height, width), anti_aliasing=True)
    
    #     return img

# Assuming you have x, y, and data arrays defined
# Example usage:
# img = _get_img_from_data(x, y, data, img_size=(257, 281))
    ...
    def get_object_img(
        ...
        **kwargs: Any,
        ...
        """
        print('in def get_object_img')
        print('self.obj_ids:', self.obj_ids)
        print('obj_id:', obj_id)
        mask = self.obj_ids == obj_id
        print('mask:', mask)
        print('self.x:', self.x)
        print('self.x[mask]:', self.x[mask])
        print('self.y:', self.y)
        print('self.y[mask]:', self.y[mask])
        x = self.x[mask]
        y = self.y[mask]
        print('x after masking in def get_object_img:', x)
        print('y after masking def get_object_img:', y)
        if data == "mpp":
            print('self.center_mpp before masking:', self.center_mpp)
            print('self.center_mpp after masking:', self.center_mpp)
            values = self.center_mpp[mask]
            if channel_ids is not None:
                values = values[:, channel_ids]
                print('values in if channel_ids is not Noneffffffffffffffffffffffffffffffffffffffffffffffff:', values)
        else:
            values = self._data[data][mask]
        if len(values.shape) == 1:
            values = values[:, np.newaxis]
        # color image if necessary
        print('fiojfiewfknmvckljfoeijrojfdlknflkdcjnodsjfoeijrfeoijfkdlnmvodsjfoiejroiewjroifdsmk:', values)
        img = MPPData._get_img_from_data(x, y, values, **kwargs)
        print('ffffffffffffffffffffffffffllllllllllllllllllllllllllllllllllllllllaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaagggggggggggggggggggggggggggggg')
        print('img in get_object_img in not if not else:', img)
        if annotation_kwargs is not None:
            if isinstance(img, tuple):  # have padding info in img
                ...
                img = (img_, img[1])
                print('img in get_object_img in if:', img)
                print('img.shape in get_object_img in if:', img.shape)
            else:
                img = annotate_img(img, from_col=data, **annotation_kwargs)
                print('img in get_object_img in else:', img)
                print('img.shape in get_object_img in else:', img.shape)
        return img
    ...
    def get_img(
        ...
        print('kwargs in get_img:', kwargs)
        """
